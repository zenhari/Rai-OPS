...
checkPageAccessWithRedirect('admin/pricing/seat_configuration/edit.php');
...
$rows = intval($config['rows'] ?? 15);
$seatsPerRow = intval($config['seats_per_row'] ?? 2);
$seatLayout = isset($_GET['seat_layout']) ? $_GET['seat_layout'] : (isset($_POST['seat_layout']) ? $_POST['seat_layout'] : '');
$rowLabels = [];

// Check if layout change is requested
$layoutChangeRequested = isset($_GET['change_layout']) && $_GET['change_layout'] == '1';
$layoutConfigured = !$layoutChangeRequested;

// Get row labels from GET or POST or generate from existing config
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['setup_layout'])) {
    ...
    $seatLayout = trim($_POST['seat_layout'] ?? '');
    $rowLabels = [];
    ...
    } elseif (empty($seatLayout)) {
        $error = 'Please select or enter a seat layout configuration.';
    } else {
        // Get custom layout if selected
        if ($seatLayout === 'custom') {
            $seatLayout = trim($_POST['custom_seat_layout'] ?? '');
            if (empty($seatLayout)) {
                $error = 'Please enter a custom seat layout.';
            }
        }
        
        if (empty($error)) {
            // Redirect to same page with layout parameters
            $params = ['id' => $configId, 'rows' => $rows, 'seats_per_row' => $seatsPerRow, 'seat_layout' => urlencode($seatLayout)];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['save_configuration'])) {
    ...
    if (empty($name)) {
        ...
        if (updateAircraftSeatConfiguration($configId, $name, $totalSeats, $rows, $seatsPerRow, $seatConfiguration)) {
...
// Prepare existing seat configuration for JavaScript
$existingSeatConfig = $config['seat_configuration'] ?? [];
$existingSeatConfigJson = json_encode($existingSeatConfig);
...
<head>
    ...
    <title>Edit Seat Configuration - <?php echo PROJECT_NAME; ?></title>
...
<body class="h-full bg-gray-50 dark:bg-gray-900">
    ...
    <div class="lg:ml-64 min-h-screen">
        <div class="flex flex-col min-h-screen">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm border-b border-gray-200 dark:border-gray-700">
                <div class="px-6 py-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Seat Configuration</h1>
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                <?php if ($layoutConfigured): ?>
                                    Configure seats: <?php echo $rows * $seatsPerRow; ?> seats (<?php echo $rows; ?> rows × <?php echo $seatsPerRow; ?> seats per row)
                                <?php else: ?>
                                    Change the seat layout dimensions
                                <?php endif; ?>
                            </p>
                        </div>
                        <div class="flex items-center gap-3">
                            <?php if ($layoutConfigured): ?>
                                <a href="edit.php?id=<?php echo $configId; ?>&change_layout=1" 
            ...
            <div class="flex-1 p-6">
                ...
                <?php if ($layoutChangeRequested || (!$layoutConfigured && count($rowLabels) != $seatsPerRow)): ?>
                    <!-- Layout Setup Form -->
                    <div class="max-w-2xl mx-auto">
                        <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg overflow-hidden">
                            ...
                            <form method="POST" class="px-6 py-6">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                    <div>
                                        ...
                                        <input type="number" 
                                               ...
                                               max="50" 
                                               required
                                               value="<?php echo isset($_POST['rows']) ? htmlspecialchars($_POST['rows']) : $rows; ?>"
                                    ...
                                    <div>
                                        ...
                                        <input type="number" 
                                               ...
                                               max="10" 
                                               required
                                               value="<?php echo isset($_POST['seats_per_row']) ? htmlspecialchars($_POST['seats_per_row']) : $seatsPerRow; ?>"
                                               class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white text-lg font-semibold">
                                        <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                                ...
                                <div class="mb-6">
                                    <label for="seat_layout" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                    ...
                                    <select id="seat_layout" 
                                            name="seat_layout" 
                                            required
                                            class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white text-base">
                                        <option value="">Select Layout</option>
                                        <option value="1" <?php echo ($seatLayout == '1') ? 'selected' : ''; ?>>1 (Single)</option>
                                        <option value="2" <?php echo ($seatLayout == '2') ? 'selected' : ''; ?>>2 (Two seats)</option>
                                        <option value="2-2" <?php echo ($seatLayout == '2-2') ? 'selected' : ''; ?>>2-2 (Two seats, Aisle, Two seats)</option>
                                        <option value="3-3" <?php echo ($seatLayout == '3-3') ? 'selected' : ''; ?>>3-3 (Three seats, Aisle, Three seats)</option>
                                        <option value="2-3-2" <?php echo ($seatLayout == '2-3-2') ? 'selected' : ''; ?>>2-3-2 (Two, Aisle, Three, Aisle, Two)</option>
                                        <option value="2-4-2" <?php echo ($seatLayout == '2-4-2') ? 'selected' : ''; ?>>2-4-2 (Two, Aisle, Four, Aisle, Two)</option>
                                        <option value="3-4-3" <?php echo ($seatLayout == '3-4-3') ? 'selected' : ''; ?>>3-4-3 (Three, Aisle, Four, Aisle, Three)</option>
                                        <option value="custom" <?php echo ($seatLayout == 'custom' || (!in_array($seatLayout, ['1', '2', '2-2', '3-3', '2-3-2', '2-4-2', '3-4-3', '']) && !empty($seatLayout))) ? 'selected' : ''; ?>>Custom (Enter manually)</option>
                                    </select>
                                    <div id="customLayoutInput" class="mt-3 <?php echo ($seatLayout == 'custom' || (!in_array($seatLayout, ['1', '2', '2-2', '3-3', '2-3-2', '2-4-2', '3-4-3', '']) && !empty($seatLayout))) ? '' : 'hidden'; ?>">
                                        <input type="text" 
                                               id="custom_seat_layout" 
                                               name="custom_seat_layout"
                                               placeholder="e.g., 2-3-2 or 3-4-3"
                                               pattern="[0-9]+(-[0-9]+)*"
                                               value="<?php echo (!in_array($seatLayout, ['1', '2', '2-2', '3-3', '2-3-2', '2-4-2', '3-4-3', 'custom', '']) && !empty($seatLayout)) ? htmlspecialchars($seatLayout) : ''; ?>"
                                               class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white text-base">
                                        <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                                            Enter layout as numbers separated by dashes (e.g., 2-3-2 means 2 seats, aisle, 3 seats, aisle, 2 seats)
                                        </p>
                                    </div>
                                    <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                                        Select how seats are arranged with aisles between them
                                    </p>
                                </div>
                                ...
                                <div class="flex justify-end space-x-3">
                                    <a href="edit.php?id=<?php echo $configId; ?>"
                    ...
                    <script>
                        ...
                        document.getElementById('seat_layout').addEventListener('change', function() {
                            updateSeatLayout();
                            updateRowLabels();
                        });
                        document.getElementById('custom_seat_layout')?.addEventListener('input', function() {
                            updateRowLabels();
                        });
                        ...
                        function updateSeatLayout() {
                            const layoutSelect = document.getElementById('seat_layout');
                            const customInput = document.getElementById('customLayoutInput');
                            const customField = document.getElementById('custom_seat_layout');
                        ...
                        function getSeatLayout() {
                            const layoutSelect = document.getElementById('seat_layout');
                            if (layoutSelect.value === 'custom') {
                                return document.getElementById('custom_seat_layout').value;
                            }
                ...
                <?php else: ?>
                    <form method="POST" id="seatConfigForm">
                        ...
                        <input type="hidden" name="seat_layout" value="<?php echo htmlspecialchars($seatLayout); ?>">
                            ...
                            <div class="px-6 py-4">
                                <div class="aircraft-container">
                                    <div class="aircraft-body" style="min-width: <?php 
                                        ...
                                        $minWidth = $rowLabelWidth + $totalSeatsWidth + $aisleWidth + 200; // +200 for padding and margins
                                    ...
                                    ?>px;">
                                        ...
                                        <div class="seat-rows-container" id="seatGrid">
                                            ...
                                            $seatGroups = [];
                                            if (!empty($seatLayout)) {
                                                if (strpos($seatLayout, '-') !== false) {
                                                    $seatGroups = array_map('intval', explode('-', $seatLayout));
                                                } else {
                                                    $seatGroups = [intval($seatLayout)];
                                                }
                                            } else {
                                                // Default: all seats in one group
                                                $seatGroups = [$seatsPerRow];
                                                ...
                                                <div class="seat-row">
                                                    ...
                                                    foreach ($seatGroups as $groupIndex => $groupSize):
                                                        ...
                                                        for ($i = 0; $i < $groupSize && $seatIndex < $seatsPerRow; $i++): 
                                                            ...
                                                            $seatData = null;
                                                            foreach ($existingSeatConfig as $sc) {
                                                                if ($sc['row'] == $rowNum && $sc['position'] == $seatLabels[$seatIndex]) {
                                                                    $seatData = $sc;
                                                                    break;
                                                                }
                                                            }
                                                    ?>
                                                            <div class="seat" 
                                                                 ...
                                                                 <?php 
                                                                 if ($seatData): 
                                                                     $isUnavailable = isset($seatData['unavailable']) && $seatData['unavailable'];
                                                                     if ($isUnavailable):
                                                                         $bgColor = '#6b7280';
                                                                         $borderColor = '#4b5563';
                                                                     else:
                                                                         $bgColor = $classColorMap[$seatData['flight_class_id']] ?? '#6b7280';
                                                                         $borderColor = $bgColor;
                                                                     endif;
                                                                 ?>
                                                                     style="background-color: <?php echo $bgColor; ?>; border-color: <?php echo $borderColor; ?>; color: #FFFFFF;"
                                                                 <?php endif; ?>>
                                                                <span class="seat-label"><?php echo $seatId; ?></span>
                                                                <?php if ($seatData): 
                                                                    $isUnavailable = isset($seatData['unavailable']) && $seatData['unavailable'];
                                                                ?>
                                                                    <br><small><?php echo $isUnavailable ? '<i class="fas fa-ban"></i> UNA' : htmlspecialchars($seatData['flight_class_code'] ?? ''); ?></small>
                                                                <?php endif; ?>
                                                            </div>
                                                        <?php 
                                                            $seatIndex++;
                                        ...
                                        <!-- Rear Section -->
                                        <div class="aircraft-rear">
                                            <div class="rear-space"></div>
                                            <div class="rear-space"></div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <input type="hidden" name="seat_configuration" id="seatConfiguration" value="<?php echo htmlspecialchars($existingSeatConfigJson); ?>">
    ...
    <script>
        ...
        const existingConfig = <?php echo $existingSeatConfigJson; ?>;
        existingConfig.forEach(seat => {
            const seatId = seat.row + seat.position;
            seatConfig[seatId] = seat;
        });
        ...
        function clearSelectedSeats() {
            selectedSeats.forEach(seatId => {
                ...
                if (seatElement) {
                    ...
                    seatElement.style.borderColor = 'rgba(255, 255, 255, 0.4)';
