...
checkPageAccessWithRedirect('admin/pricing/flight_class/add.php');
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($name)) {
        ...
        $db = getDBConnection();
        $stmt = $db->prepare("SELECT id FROM flight_classes WHERE code = ?");
        $stmt->execute([strtoupper($code)]);
        if ($stmt->fetch()) {
            $error = 'Flight Class Code already exists.';
        } else {
            if (addFlightClass($name, $code, !empty($description) ? $description : null, $status)) {
                $message = 'Flight class added successfully.';
                // Redirect after 1 second
                header('Refresh: 1; url=index.php');
            } else {
                $error = 'Failed to add flight class.';
            }
        }
    }
}
...
<head>
    ...
    <title>Add Flight Class - <?php echo PROJECT_NAME; ?></title>
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm border-b border-gray-200 dark:border-gray-700">
                <div class="px-6 py-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Add New Flight Class</h1>
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Create a new flight class configuration</p>
                        </div>
                        <a href="index.php" 
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="description" name="description" rows="3"
                                          ...
                                          class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white"><?php echo isset($_POST['description']) ? htmlspecialchars($_POST['description']) : ''; ?></textarea>
                            </div>
