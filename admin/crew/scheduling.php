...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'duplicate_crew') {
        ...
        ob_end_clean();
    } elseif ($_POST['action'] === 'batch_update_crew') {
        ...
        try {
            ...
            if (empty($updates) || !is_array($updates)) {
                ...
                foreach ($updates as $update) {
                    $flightId = intval($update['flight_id'] ?? 0);
                    $crewData = $update['crew_data'] ?? [];
                    
                    if ($flightId > 0 && !empty($crewData)) {
                ...
                if ($successCount > 0) {
                    $message = "Successfully updated {$successCount} flight(s).";
                    if ($failCount > 0) {
                        $message .= " Failed to update {$failCount} flight(s).";
                    }
                    ...
                    $error = 'Failed to update any flights.';
                    if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
                        ob_clean();
                        header('Content-Type: application/json');
                        echo json_encode(['success' => false, 'error' => $error]);
...
<head>
    ...
    <style>
        ...
        #saveStatusMessages::-webkit-scrollbar {
            width: 6px;
        }
        
        #saveStatusMessages::-webkit-scrollbar-track {
            background: #f7fafc;
            border-radius: 3px;
        }
        
        #saveStatusMessages::-webkit-scrollbar-thumb {
            background: #cbd5e0;
            border-radius: 3px;
        }
        
        .dark #saveStatusMessages {
            scrollbar-color: #4a5568 #2d3748;
        }
        
        .dark #saveStatusMessages::-webkit-scrollbar-track {
            background: #2d3748;
        }
        
        .dark #saveStatusMessages::-webkit-scrollbar-thumb {
            background: #4a5568;
        }
        ...
        /* Rego grouping styles */
        .rego-group {
            position: relative;
        }
        .rego-group-first td:first-child {
            border-left: 3px solid #3b82f6;
            border-top: 3px solid #3b82f6;
            border-top-left-radius: 0.5rem;
        }
        .rego-group-first td:last-child {
            border-right: 3px solid #3b82f6;
            border-top: 3px solid #3b82f6;
            border-top-right-radius: 0.5rem;
        }
        .rego-group-first td:not(:first-child):not(:last-child) {
            border-top: 3px solid #3b82f6;
        }
        .rego-group-middle td:first-child {
            border-left: 3px solid #3b82f6;
        }
        .rego-group-middle td:last-child {
            border-right: 3px solid #3b82f6;
        }
        .rego-group-last td:first-child {
            border-left: 3px solid #3b82f6;
            border-bottom: 3px solid #3b82f6;
            border-bottom-left-radius: 0.5rem;
        }
        .rego-group-last td:last-child {
            border-right: 3px solid #3b82f6;
            border-bottom: 3px solid #3b82f6;
            border-bottom-right-radius: 0.5rem;
        }
        .rego-group-last td:not(:first-child):not(:last-child) {
            border-bottom: 3px solid #3b82f6;
        }
        .rego-group-last {
            margin-bottom: 0.75rem;
        }
        .rego-group-single td:first-child {
            border-left: 3px solid #3b82f6;
            border-top: 3px solid #3b82f6;
            border-bottom: 3px solid #3b82f6;
            border-top-left-radius: 0.5rem;
        ...
        .rego-group-single td:last-child {
            border-right: 3px solid #3b82f6;
            border-top: 3px solid #3b82f6;
            border-bottom: 3px solid #3b82f6;
            border-top-right-radius: 0.5rem;
        ...
        .rego-group-single td:not(:first-child):not(:last-child) {
            border-top: 3px solid #3b82f6;
            border-bottom: 3px solid #3b82f6;
        }
        .rego-group-single {
        ...
        .dark .rego-group-first td,
        .dark .rego-group-middle td,
        .dark .rego-group-last td,
        .dark .rego-group-single td {
            border-color: #60a5fa;
        }
    </style>
</head>
<body class="h-full bg-gray-50 dark:bg-gray-900">
                            ...
                            <div class="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 flex-1 flex flex-col min-h-0 overflow-hidden">
                                ...
                                <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
                                    <div class="flex items-center space-x-4">
                                        ...
                                        <button id="addCrewColumnBtn" onclick="addCrewColumn()" 
                                        ...
                                        <span id="pendingChangesCount" class="text-sm text-gray-600 dark:text-gray-400 hidden">
                                            <span id="changesCount">0</span> pending change(s)
                                        </span>
                                    </div>
                                    <button onclick="discardAllChanges()" 
                                            class="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 text-sm font-medium rounded-md text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed hidden"
                                            id="discardAllBtn">
                                ...
                                <table class="min-w-full crew-table">
                                    <thead class="bg-gray-50 dark:bg-gray-700">
                                        <tr>
                                            ...
                                            <?php for ($i = 1; $i <= 10; $i++): ?>
                                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider crew-cell crew-header-<?php echo $i; ?> <?php echo $i > 4 ? 'hidden' : ''; ?>">Crew<?php echo $i; ?></th>
                                            <?php endfor; ?>
                                        </tr>
                                    </thead>
                                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                        <?php 
                                        // Convert to numerically indexed array for easier access
                                        $flightsArray = array_values($flights);
                                        $prevRego = null;
                                        
                                        // Render flights with grouping classes
                                        foreach ($flightsArray as $index => $flight): 
                                            $currentRego = $flight['Rego'] ?? '';
                                            $isFirstInGroup = ($prevRego === null || $prevRego !== $currentRego);
                                            
                                            // Check if next flight has different Rego
                                            $nextRego = null;
                                            if (isset($flightsArray[$index + 1])) {
                                                $nextRego = $flightsArray[$index + 1]['Rego'] ?? '';
                                            }
                                            $isLastInGroup = ($nextRego === null || $nextRego !== $currentRego);
                                            
                                            // Determine group class
                                            $groupClass = '';
                                            if ($isFirstInGroup && $isLastInGroup) {
                                                $groupClass = 'rego-group-single';
                                            } elseif ($isFirstInGroup) {
                                                $groupClass = 'rego-group-first';
                                            } elseif ($isLastInGroup) {
                                                $groupClass = 'rego-group-last';
                                            } else {
                                                $groupClass = 'rego-group-middle';
                                            }
                                            
                                            $prevRego = $currentRego;
                                        ?>
                                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 rego-group <?php echo $groupClass; ?>">
                                                <!-- Dup Button -->
                                                ...
                                                ?>
                                                    <!-- Crew<?php echo $i; ?> -->
                                                <td class="px-4 py-4 crew-cell crew-cell-<?php echo $i; ?> <?php echo $i > 4 ? 'hidden' : ''; ?>" data-crew-number="<?php echo $i; ?>">
                                                    <div class="space-y-1">
                                                        <!-- Crew Autocomplete -->
    ...
    <!-- Save Progress Modal -->
    <div id="saveProgressModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
        <div class="relative top-20 mx-auto p-5 border w-11/12 max-w-2xl shadow-lg rounded-md bg-white dark:bg-gray-800">
            <div class="mt-3">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3 overflow-hidden">
                            <div id="progressBar" class="bg-gradient-to-r from-green-500 to-green-600 h-3 rounded-full transition-all duration-300 ease-out" style="width: 0%"></div>
                        </div>
                    </div>
                    
                    <!-- Status Messages -->
                    <div id="saveStatusMessages" class="space-y-2 max-h-64 overflow-y-auto">
                        <!-- Status messages will be added here -->
                    </div>
                    
                    <!-- Summary -->
                    <div id="saveSummary" class="hidden mt-4 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 rounded-lg">
                        <div class="flex items-center">
                            <i class="fas fa-check-circle text-green-600 dark:text-green-400 mr-2"></i>
                            <div>
                                <p class="text-sm font-medium text-green-800 dark:text-green-200">
                                    <span id="successCount">0</span> flight(s) saved successfully
                                </p>
                                <p id="failSummary" class="text-sm text-green-700 dark:text-green-300 mt-1 hidden">
                                    <span id="failCount">0</span> flight(s) failed to save
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="mt-6 flex justify-end">
                    <button id="closeProgressModalBtn" onclick="closeSaveProgressModal()" 
                            class="px-4 py-2 bg-gray-300 dark:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-md hover:bg-gray-400 dark:hover:bg-gray-500 hidden">
                        Close
                    </button>
                </div>
            </div>
    ...
    <script>
        ...
        // Store pending changes (flight_id -> {crew_data: {...}})
        let pendingChanges = {};
        
        // Track original values to detect changes
        let originalValues = {};
        
        // Track the last visible crew column (4 = Crew4 is always visible, max is 10)
        let lastVisibleCrewColumn = 4;
        ...
        function openDuplicateModal(flightId, flightNo) {
            ...
            if (flightsList.length === 0) {
                ...
                flightsList.forEach(flight => {
                    ...
                    // Get flight number - try FlightNo first, then TaskName, then id
                    const flightNo = flight.FlightNo || flight.TaskName || `Flight ${flight.id}` || 'N/A';
                    
                    row.innerHTML = `
                        <td class="px-4 py-2">
                            <input type="checkbox" class="flight-checkbox" value="${flight.id}" onchange="updateSelectedCount()">
                        </td>
                        <td class="px-4 py-2 text-sm text-gray-900 dark:text-white">${flightNo}</td>
                        <td class="px-4 py-2 text-sm text-gray-900 dark:text-white">${flight.aircraft_registration || flight.Rego || 'N/A'}</td>
        ...
        function duplicateCrew() {
            ...
            // Get source flight crew data from input fields on the page
            // First check if source flight has pending changes, otherwise use current values
            let sourceCrewData = {};
            
            // If source flight has pending changes, use those
            if (pendingChanges[currentSourceFlightId] && pendingChanges[currentSourceFlightId].crew_data) {
                sourceCrewData = { ...pendingChanges[currentSourceFlightId].crew_data };
            } else {
                // Otherwise, get from input fields
                for (let i = 1; i <= 10; i++) {
                    const crewField = `Crew${i}`;
                    const roleField = `Crew${i}_role`;
                    
                    // Get crew value from input field
                    const crewInput = document.getElementById(`crew_${currentSourceFlightId}_${crewField}`);
                    if (crewInput && crewInput.dataset.selectedId) {
                        const userId = crewInput.dataset.selectedId;
                        if (userId && userId !== '') {
                            sourceCrewData[crewField] = userId;
                        }
                    }
                    
                    // Get role value from input field
                    const roleInput = document.getElementById(`role_${currentSourceFlightId}_${roleField}`);
                    if (roleInput && roleInput.value) {
                        sourceCrewData[roleField] = roleInput.value;
                    }
                }
            }
            
            // Add crew data to pendingChanges for each selected target flight
            let addedCount = 0;
            selectedFlights.forEach(targetFlightId => {
                if (!pendingChanges[targetFlightId]) {
                    pendingChanges[targetFlightId] = { crew_data: {} };
                }
                
                // Merge source crew data into pending changes
                Object.assign(pendingChanges[targetFlightId].crew_data, sourceCrewData);
                addedCount++;
            });
            
            // Update UI inputs to show the duplicated crew data
            selectedFlights.forEach(targetFlightId => {
                for (let i = 1; i <= 10; i++) {
                    const crewField = `Crew${i}`;
                    const roleField = `Crew${i}_role`;
                    ...
                    // Update crew input
                    const crewInput = document.getElementById(`crew_${targetFlightId}_${crewField}`);
                    if (crewInput && sourceCrewData[crewField]) {
                        // Find user name from users list
                        const userId = sourceCrewData[crewField];
                        const user = allUsers.find(u => u.id == userId);
                        if (user) {
                            crewInput.value = user.first_name + ' ' + user.last_name;
                            crewInput.dataset.selectedId = userId;
                            
                        }
                    }
                    
                    // Update role input
                    const roleInput = document.getElementById(`role_${targetFlightId}_${roleField}`);
                    if (roleInput && sourceCrewData[roleField]) {
                        roleInput.value = sourceCrewData[roleField];
                    }
                }
            });
            ...
            showMessage(`Crew assignments prepared for ${addedCount} flight(s). Click "Save All Changes" to save.`, 'success');
        }
        ...
        function clearCrewInput(flightId, field) {
            ...
            // Hide next crew columns if this one is cleared
            hideNextCrewColumns(flightId, field);
        }
        ...
        function updateCrew(flightId, field, value) {
            // Store change in pendingChanges instead of updating immediately
            if (!pendingChanges[flightId]) {
                pendingChanges[flightId] = { crew_data: {} };
            }
            
            // Convert empty string to null for consistency
            const finalValue = (value === '' || value === null || value === undefined) ? null : String(value);
            pendingChanges[flightId].crew_data[field] = finalValue;
        ...
        // Function to update pending changes UI
        function updatePendingChangesUI() {
            ...
            const pendingChangesCount = document.getElementById('pendingChangesCount');
            const saveAllBtn = document.getElementById('saveAllBtn');
            const discardAllBtn = document.getElementById('discardAllBtn');
            
            if (totalChanges > 0) {
                changesCountSpan.textContent = totalChanges;
                pendingChangesCount.classList.remove('hidden');
                saveAllBtn.disabled = false;
                discardAllBtn.classList.remove('hidden');
            } else {
                pendingChangesCount.classList.add('hidden');
                saveAllBtn.disabled = true;
                discardAllBtn.classList.add('hidden');
            }
        }
        ...
        function saveAllChanges() {
            ...
            const updates = [];
            const flightIds = [];
            for (const flightId in pendingChanges) {
                updates.push({
                    flight_id: parseInt(flightId),
                    crew_data: pendingChanges[flightId].crew_data
                });
                flightIds.push(parseInt(flightId));
            }
            
            // Show progress modal
            openSaveProgressModal(flightIds.length);
            ...
            formData.append('action', 'batch_update_crew');
            ...
            .then(data => {
                ...
                if (data.success) {
                    // Parse success message to get counts
                    const successMatch = data.message.match(/(\d+)\s+flight\(s\)/);
                    const failMatch = data.message.match(/Failed to update\s+(\d+)\s+flight\(s\)/);
                    
                    const successCount = successMatch ? parseInt(successMatch[1]) : flightIds.length;
                    const failCount = failMatch ? parseInt(failMatch[1]) : 0;
                    
                    // Show summary
                    showSaveSummary(successCount, failCount);
                    
                    // Add success messages with delay
                    addStatusMessage('Saving flight data...', 'info');
                    setTimeout(() => {
                        flightIds.forEach((flightId, index) => {
                            setTimeout(() => {
                                addStatusMessage(`Flight ${flightId} saved successfully`, 'success');
                            }, (index + 1) * 100);
                        });
                    }, 200);
                    ...
                    // Show close button and reload after delay
                    setTimeout(() => {
                        document.getElementById('closeProgressModalBtn').classList.remove('hidden');
                        setTimeout(() => {
                            location.reload();
                        }, 2000);
                    }, 1000);
                } else {
                    addStatusMessage(data.error || 'Failed to save changes', 'error');
                    document.getElementById('closeProgressModalBtn').classList.remove('hidden');
            ...
            .catch(error => {
                ...
                addStatusMessage('Error saving changes: ' + error.message, 'error');
                document.getElementById('closeProgressModalBtn').classList.remove('hidden');
                saveBtn.disabled = false;
            });
        }
        
        // Function to open save progress modal
        function openSaveProgressModal(totalFlights) {
            const modal = document.getElementById('saveProgressModal');
            const statusMessages = document.getElementById('saveStatusMessages');
            ...
            // Add initial message
            addStatusMessage(`Preparing to save ${totalFlights} flight(s)...`, 'info');
        }
        
        // Function to close save progress modal
        ...
        // Function to update progress percentage text
        ...
        function addStatusMessage(message, type) {
            const statusMessages = document.getElementById('saveStatusMessages');
            const messageDiv = document.createElement('div');
            messageDiv.className = 'flex items-center p-2 rounded text-sm';
            ...
            if (type === 'success') {
                ...
                bgClass = 'bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700';
                textClass = 'text-green-800 dark:text-green-200';
            } else if (type === 'error') {
                iconClass = 'fas fa-exclamation-circle text-red-600 dark:text-red-400';
                bgClass = 'bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700';
                textClass = 'text-red-800 dark:text-red-200';
            } else {
                iconClass = 'fas fa-info-circle text-blue-600 dark:text-blue-400';
                bgClass = 'bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700';
                textClass = 'text-blue-800 dark:text-blue-200';
            }
            
            messageDiv.className += ' ' + bgClass;
            messageDiv.innerHTML = `
        ...
        // Function to show save summary
        function showSaveSummary(successCount, failCount) {
            const summary = document.getElementById('saveSummary');
            document.getElementById('successCount').textContent = successCount;
            
            if (failCount > 0) {
                document.getElementById('failCount').textContent = failCount;
                document.getElementById('failSummary').classList.remove('hidden');
            } else {
                document.getElementById('failSummary').classList.add('hidden');
            }
        ...
        // Prevent closing progress modal when clicking outside (during save)
        document.getElementById('saveProgressModal').addEventListener('click', function(e) {
            if (e.target === this) {
                // Don't close modal during save operation
                const closeBtn = document.getElementById('closeProgressModalBtn');
                if (!closeBtn.classList.contains('hidden')) {
                    // Only allow closing if save is complete
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Initialize crew column visibility based on existing data
            initializeCrewColumnVisibility();
        });
        
        // Function to add a new crew column (Crew5-Crew10)
        function addCrewColumn() {
            if (lastVisibleCrewColumn >= 10) {
                // All columns are already visible
                return;
            }
            
            // Show the next crew column
            lastVisibleCrewColumn++;
            const crewNumber = lastVisibleCrewColumn;
            
            // Show header for this crew column
            const header = document.querySelector('.crew-header-' + crewNumber);
            if (header) {
                header.classList.remove('hidden');
            }
            
            // Show all body cells for this crew column across all flight rows
            const cells = document.querySelectorAll('.crew-cell-' + crewNumber);
            cells.forEach(cell => {
                cell.classList.remove('hidden');
            });
            
            // Disable button if we've reached Crew10
            if (lastVisibleCrewColumn >= 10) {
                const addBtn = document.getElementById('addCrewColumnBtn');
                if (addBtn) {
                    addBtn.disabled = true;
                    addBtn.innerHTML = '<i class="fas fa-check mr-2"></i>All Columns Added';
                }
            }
        }
        
        // Function to hide next crew columns when current one is cleared
        function hideNextCrewColumns(flightId, field) {
            // Extract crew number from field (e.g., "Crew5" -> 5)
            const crewNumber = parseInt(field.replace('Crew', ''));
            
            // Hide all columns from this one onwards (Crew5-10) if they have no data
            let newLastVisible = 4; // Crew4 is always visible
            
            for (let i = 5; i <= 10; i++) {
                // Check if this column has any data
                const hasData = checkCrewColumnHasData(i);
                
                if (!hasData && i > 4) {
                    // Hide header
                    const header = document.querySelector('.crew-header-' + i);
                    if (header) {
                        header.classList.add('hidden');
                    }
                    
                    // Hide all body cells for this crew column
                    const cells = document.querySelectorAll('.crew-cell-' + i);
                    cells.forEach(cell => {
                        cell.classList.add('hidden');
                    });
                } else if (hasData) {
                    // This column has data, so it should be visible
                    newLastVisible = i;
                }
            }
            
            // Update lastVisibleCrewColumn
            lastVisibleCrewColumn = newLastVisible;
            ...
            if (addBtn) {
                if (lastVisibleCrewColumn >= 10) {
                    ...
                    addBtn.innerHTML = '<i class="fas fa-plus mr-2"></i>Add Crew Column';
                }
            }
        }
        
        // Function to check if a crew column has any data across all flights
        function checkCrewColumnHasData(crewNumber) {
            const field = 'Crew' + crewNumber;
            const inputs = document.querySelectorAll(`input[data-field="${field}"]`);
            
            for (let input of inputs) {
                if (input.dataset.selectedId && input.dataset.selectedId !== '') {
                    return true;
                }
            }
        ...
        // Function to initialize crew column visibility based on existing data
        function initializeCrewColumnVisibility() {
            // Check each crew column from 5 to 10
            let maxVisibleColumn = 4; // Crew4 is always visible
            
            for (let i = 5; i <= 10; i++) {
                const hasData = checkCrewColumnHasData(i);
                
                if (hasData) {
                    // Show this column and all previous columns
                    for (let j = 5; j <= i; j++) {
                        // Show header
                        const header = document.querySelector('.crew-header-' + j);
                        if (header) {
                            header.classList.remove('hidden');
                        }
                        
                        // Show all body cells
                        const cells = document.querySelectorAll('.crew-cell-' + j);
                        cells.forEach(cell => {
                            cell.classList.remove('hidden');
                        });
                    }
            ...
            // Update lastVisibleCrewColumn to reflect the current state
            lastVisibleCrewColumn = maxVisibleColumn;
