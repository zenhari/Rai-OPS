...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'get_flight_data') {
    ...
    try {
        ...
        $isDomestic = isset($_POST['domestic']);
        ...
        // Add domestic/international filter (simplified - you may need to adjust based on your data structure)
        if ($isDomestic && !$isInternational) {
            // Add logic to identify domestic flights (e.g., specific route patterns)
            $whereConditions[] = "Route NOT LIKE '%-INTL%'";
        } elseif ($isInternational && !$isDomestic) {
            $whereConditions[] = "Route LIKE '%-INTL%'";
        }
        
        // Exclude cancelled flights
        $whereConditions[] = "ScheduledTaskStatus NOT LIKE 'Cancelled'";
        
        $whereClause = !empty($whereConditions) ? 'WHERE ' . implode(' AND ', $whereConditions) : '';
        
        $query = "SELECT 
                    Route,
                    SUM(CAST(adult AS UNSIGNED)) as total_adult,
                    SUM(CAST(child AS UNSIGNED)) as total_child,
                    SUM(CAST(infant AS UNSIGNED)) as total_infant,
...
<head>
    ...
    <title>City-Pairs Report - <?php echo PROJECT_NAME; ?></title>
...
<body class="h-full bg-gray-50 dark:bg-gray-900">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm border-b border-gray-200 dark:border-gray-700">
                <div class="px-6 py-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">City-Pairs Report</h1>
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Monthly City-Pairs and Traffic Report</p>
                        </div>
                    </div>
                </div>
            </div>
                                ...
                                <div class="space-y-2 mb-4">
                                    ...
                                    <div class="flex space-x-4 h-10 items-center">
                                        <div class="flex items-center">
                                            <input type="checkbox" id="domestic" class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 dark:bg-gray-700 dark:border-gray-600">
                                            <label for="domestic" class="ml-2 text-sm text-gray-700 dark:text-gray-300">DOM</label>
                                        </div>
                                        <div class="flex items-center">
                                            <input type="checkbox" id="international" class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 dark:bg-gray-700 dark:border-gray-600">
                                            <label for="international" class="ml-2 text-sm text-gray-700 dark:text-gray-300">INTL</label>
                                        </div>
                                    </div>
                                </div>
    ...
    <script>
        ...
        function loadFlightData() {
            ...
            formData.append('domestic', isDomestic);
            formData.append('international', isInternational);
