-- Update journey_log_entries table to add missing fields
-- This script adds fields needed for the new journey log form structure
-- Note: Run this script carefully. If a column already exists, you'll get an error which you can ignore.

-- First, check if log_date column exists, if not, we'll add selected_date after pilot_name
-- Add missing fields to journey_log_entries table
-- Check if columns exist before adding (MySQL doesn't support IF NOT EXISTS in ALTER TABLE)
-- You may need to run these one by one and ignore errors for existing columns

-- Add selected_date after pilot_name (or log_date if it exists)
